...
ED_Bitmap::ED_Bitmap(int width, int height)
{
	...
	this->m_surface = SDL_CreateRGBSurfaceWithFormatFrom((void*)m_buffer, width, height,
...
ED_Color* ED_Bitmap::get_pixel(int x, int y)
{
	...
	ED_Color* cr = new ED_Color((pixel >> this->m_surface->format->Rshift) & 0xFF,
		(pixel >> this->m_surface->format->Gshift) & 0xFF,
		(pixel >> this->m_surface->format->Bshift) & 0xFF,
		(pixel >> this->m_surface->format->Ashift) & 0xFF);
